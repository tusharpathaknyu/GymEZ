...
export class ChallengeService {
  ...
  static async updateChallengeProgress(
    ...
    const isCompleted = challenge?.target_value ? progress >= challenge.target_value : false;
  ...
  static async addPoints(userId: string, points: number, source: string): Promise<UserPoints> {
  ...
  static async awardBadge(userId: string, badgeId: string, challengeId?: string): Promise<UserBadge> {
    ...
    if (userBadge.badge?.points_value) {
      await this.addPoints(userId, userBadge.badge.points_value, 'badge_earned');
    }
  ...
  static async getLeaderboard(
    type: 'global_points' | 'monthly_points' | 'challenge_specific' | 'gym_specific',
    referenceId?: string,
    limit: number = 50
  ...
  private static async updateChallengeLeaderboards(): Promise<void> {
    ...
    for (const challenge of activeChallenges || []) {
      ...
      const leaderboardEntries = challengeParticipants?.map((participant, index) => ({
  ...
  private static async awardChallengeCompletion(challengeId: string, userId: string): Promise<void> {
    ...
    if (challenge.reward_points) {
      await this.addPoints(userId, challenge.reward_points, 'challenge_completion');
    }
  ...
  private static async checkLevelUpBadges(userId: string, newLevel: number): Promise<void> {
