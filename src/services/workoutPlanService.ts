...
export class WorkoutPlanService {
  ...
  static async getWorkoutPlan(
    id: string,
  ): Promise<WorkoutPlan & { sessions: WorkoutSession[] }> {
  ...
  static async getWorkoutSession(
    id: string,
  ): Promise<WorkoutSession & { exercises: SessionExercise[] }> {
    const { data, error } = await supabase
      .from('workout_sessions')
      .select(
        `
  ...
  static async getUserWorkoutHistory(
    userId?: string,
    limit: number = 20,
  ): Promise<UserWorkoutLog[]> {
    const {
  ...
  static async getProgressStats(userId?: string): Promise<{
    ...
    const { data: workouts, error } = await supabase
      .from('user_workout_logs')
      .select(
        'duration_minutes, completed_at, exercise_logs(exercise_library_id, exercise:exercise_library(name))',
      )
      .eq('user_id', targetUserId)
    ...
    const totalDuration =
      workouts?.reduce((sum, w) => sum + (w.duration_minutes || 0), 0) || 0;
  ...
  > {
    ...
    const progress =
      data?.map(log => {
        ...
        const totalVolume = (log.reps_completed || []).reduce(
          (sum: number, reps: number, index: number) => {
        ...
        const bestSet = (log.reps_completed || []).reduce(
          (
            best: { reps: number; weight: number },
            reps: number,
            index: number,
          ) => {
            const weight = (log.weights_used || [])[index] || 0;
...
- Main exercises: ${suggestedExercises.length} exercises, ${
      config.sets
    } sets each
...
- Estimated duration: ${durationMinutes} minutes
    ...
    return {
      suggestedExercises: suggestedExercises.slice(
        0,
        Math.floor(durationMinutes / 10),
      ), // Rough exercise count based on time
      workoutStructure,
    };
  }
}
